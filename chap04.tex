...
A {\em compound statement} is a statement, like {\tt if} and {\tt for}, that
contains other statements in an indented body.
If you put one compound statement in the body of another, they are {\em nested}.

A {\em scalar} is single value; a {\em vector} is a sequence of values, and a {\em matrix} is a two-dimensional collection of values (also called {\em array} in some MATLAB documentation).

An {\em index} is an integer value used to indicate one of the elements
in a vector or matrix (also called {\em subscript} in some MATLAB documentation).
