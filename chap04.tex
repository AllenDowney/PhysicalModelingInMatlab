...
This chapter also presents conditional statements, and three patterns for working with data: reduce, apply, and search.
...
it is set of numbers that correspond to positive integers.
...
Variables that contain vectors are often capital letters.
That's just a convention; MATLAB doesn't require it, but for beginning programmers, it is a useful way to remember what is a vector and what is a scalar.
...
In MATLAB, everything is a matrix (except strings).
...
According to MATLAB, everything is a {\tt double}, which
...
zero or negative, you get an error:
...
setting with a style string, as we saw in Section~\ref{plotting}.
