...
The interpreter is the program that reads and executes MATLAB or Octave code.
The interpreter prints a prompt to indicate that it's waiting for you to type a command.
A command is a line of code executed by the interpreter.

An expression is a sequence of operands and operators that specifies
a mathematical computation and yields a value.
An operator is a symbol, like \lstinline{*} and \lstinline{+}, that
represents a mathematical operation.
An operand is a number or variable that appears in an expression along
with operators.

A function is a named computation; for example \lstinline{log10} is the
name of a function that computes logarithms in base 10.
A function call is a statement that causes a function to execute and compute a result.
An argument is an expression that appears in a function call to
specify the value the function operates on.

A variable is a named value. An assignment statement is the command that creates a new variable
(if necessary) and gives it a value.
A workspace is a set of variables and their values.

A string is a value that consists of a sequence of characters (as opposed to a number).
