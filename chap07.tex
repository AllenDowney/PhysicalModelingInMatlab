...
Since the value we assigned to {\tt sin} is a number, and a number is considered a 1x1 matrix, MATLAB tries to access the 5th element of the matrix and finds that there isn't one.
...
In this chapter I introduced {\tt fzero}, a function we can use to solve nonlinear equations.
...
Other wise, we can use a {\em numerical method}, which finds a {\em numerical solution} to the equation, which is usually an approximation.

To solve nonlinear equations, we often rewrite them as functions and then find one or more {\em zeroes} of the function, that is, arguments that make the value of the function $0$.
