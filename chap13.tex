...
In Section~\ref{projectile} we plotted {\tt X} and {\tt Y} separately as functions of time.  As an alternative, we can plot them against each other, like this:
...
The {\tt fminsearch} function is similar to {\tt fzero}, which we saw in Section\ref{fzero}.  Recall that {\tt fzero} takes a function handle and an initial guess, and returns a root of the function.
...
If you're curious about how {\tt fminsearch} works, see Section~\ref{howfminsearch}.
...
\emph{https://en.wikipedia.org/wiki/Magnus\_effect}), which is
