...
As an example of Newtonian motion, let's go back to the question from \nameref{penny} on page~\pageref{penny}:
...
\linebreak If \lstinline{direction} is negative, the event only occurs if \lstinline{value} is decreasing.
...
Lastly, we compute total acceleration due to drag and \mbox{gravity~\wingding{3}}.
...
In this version, we call \lstinline{acceleration} to compute the total acceleration due to gravity and drag \wingding{1}.
